//! Use case: spawn an SFTP download and return the initial transfer snapshot.
//!
//! Translates the v3 `ssh_download_impl` (from `src/mcp/tools/sftp.rs`) into
//! a hexagonal use case orchestrating ports. No rmcp / russh / russh-sftp /
...
//!    `SSH_DOWNLOAD: ERROR / REASON: [SESSION_NOT_FOUND]` block.
...
//! 4. Hand the work to [`SftpClientPort::download`] — the adapter spawns the
//!    streaming task internally, opens the SFTP session, and returns a
//!    [`crate::domain::transfer::TransferEntity`] in
//!    [`crate::domain::transfer::TransferStatus::Running`] populated with the
//!    resolved local path, the remote path, and (when available) the total
//!    byte count surfaced by the adapter's metadata probe.
...
pub struct DownloadRequest {
    ...
    /// Remote source path on the SSH server.
    pub remote_path: String,
    /// Local destination path. Tilde expansion / relative-to-home resolution
...
pub struct DownloadOutcome {
    ...
    pub session_id: SessionId,
    /// Optional agent the session was bound to at the time of the download.
    pub agent_id: Option<AgentId>,
    /// Resolved local path returned by the SFTP adapter (after tilde / home
    /// expansion).
    pub local_path: String,
    /// Remote source path (echoed unchanged).
    pub remote_path: String,
    /// Total bytes the remote file holds at start time. `0` when the
    /// adapter could not resolve the metadata before spawning the task —
...
/// Download-file use case generic over every adapter dependency. The
/// composition root pins concrete adapter types per binary; tests inject
/// fakes via [`crate::adapters`].
#[derive(Debug)]
pub struct DownloadFileUseCase<F, SR, TR, C, I, Cfg, Sub>
where
    ...
    Sub: SubscriberRegistryPort + Send + Sync,
{
    ...
    ///   cap is reached.
    /// - Any [`DomainError`] surfaced by [`SftpClientPort::download`] (e.g.
    ///   `Sftp` for remote-metadata / `SftpOpenFailed` errors,
    ///   `SessionNotFound` when the russh handle was already torn down
    ///   between the session probe and the SFTP call).
...
#[cfg(test)]
mod tests {
    use super::{DownloadFileUseCase, DownloadRequest};
    ...
    /// Identical shape to the upload-file test double — kept local so each
    /// use case test module stays self-contained.
    ...
    type UseCaseUnderTest = DownloadFileUseCase<
        FakeSftpClient,
        DashMapSessionRepo,
        DashMapTransferRepo,
        FakeClock,
    ...
    fn base_request(session_id: SessionId) -> DownloadRequest {
        DownloadRequest {
            session_id,
    ...
    async fn happy_path_returns_download_outcome_with_running_status_persisted() {
        let wired = build_wired();
        let seeded = seed_session(&wired.sessions, "sess-1", None);
        wired.sftp.queue_download_ok(8192);
        ...
        assert_eq!(outcome.local_path, "/tmp/dest.bin");
        assert_eq!(outcome.remote_path, "/srv/source.bin");
        assert_eq!(outcome.total_bytes, 8192);
        assert!(
        ...
        assert_eq!(stored.direction, TransferDirection::Download);
    }
    ...
    async fn max_transfers_exceeded_short_circuits_with_explicit_error() {
        ...
        let err = wired
            ...
            .await
            .expect_err("download must fail at the cap");
    ...
    async fn agent_id_propagates_into_outcome() {
        let wired = build_wired();
        let agent = AgentId::new("claude-agent-13".to_string());
        let seeded = seed_session(&wired.sessions, "sess-agent", Some(agent.clone()));
        wired.sftp.queue_download_ok(2048);
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn sftp_open_failed_error_is_surfaced_to_caller() {
        let wired = build_wired();
        let seeded = seed_session(&wired.sessions, "sess-open", None);
        wired.sftp.queue_download_error(DomainError::Sftp(
            "open sftp session: channel closed".to_string(),
        ));
        ...
        assert_eq!(
            err,
            DomainError::Sftp("open sftp session: channel closed".to_string())
        );
    }
    ...
    async fn subscriber_poke_fires_for_transfer_resource_kind() {
        ...
        wired.sftp.queue_download_ok(64);
    ...
    async fn sftp_call_carries_the_request_paths_verbatim() {
        ...
        wired.sftp.queue_download_ok(128);

        let mut req = base_request(seeded.id.clone());
        req.remote_path = "/var/log/app.log".to_string();
        req.local_path = "/tmp/downloads/app.log".to_string();
        let outcome = wired.uc.execute(req).await.expect("download ok");

        let calls = wired.sftp.calls();
        assert_eq!(calls.len(), 1);
        match &calls[0] {
            FakeSftpCall::Download {
                transfer_id,
                request,
            } => {
                assert_eq!(transfer_id, &outcome.transfer_id);
                assert_eq!(request.session_id, seeded.id);
                assert_eq!(request.remote_path, "/var/log/app.log");
                assert_eq!(request.local_path, "/tmp/downloads/app.log");
            }
            FakeSftpCall::Upload { .. } | FakeSftpCall::Cancel { .. } => {
                panic!("expected Download, got {:?}", calls[0])
            }
        }
    }
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn second_download_in_same_session_increments_counter_under_cap() {
