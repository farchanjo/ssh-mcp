...
/// declaration order so the LLM scans them top-down. The five workflows
/// mirror the v4.7 task list.
...
mod tests {
    ...
    #[test]
    fn prompts_catalog_lists_five_entries() {
        let prompts = list_prompts();
        assert_eq!(prompts.len(), 5);
        let names: Vec<&str> = prompts.iter().map(|p| p.name.as_str()).collect();
        assert!(names.contains(&"run_one_shot_command"));
        assert!(names.contains(&"investigate_session"));
        assert!(names.contains(&"upload_and_verify"));
        assert!(names.contains(&"interactive_shell_drive"));
        assert!(names.contains(&"cleanup_agent"));
    }
