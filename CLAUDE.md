...
- **ssh-mcp-stdio** (`src/bin/ssh_mcp_stdio.rs`): Stdio-based MCP transport for direct LLM integration

### Core Modules (`src/mcp/`)
| Module | Lines | Description |
|--------|-------|-------------|
| **types.rs** | ~280 | Response types (`SessionInfo`, `SshConnectResponse`, etc.) |
| **config.rs** | ~600 | Constants and configuration resolution |
| **error.rs** | ~360 | Error classification for retry logic |
| **session.rs** | ~90 | Session storage and `SshClientHandler` |
| **client.rs** | ~640 | SSH connection, authentication, command execution |
| **forward.rs** | ~160 | Port forwarding (feature-gated) |
| **commands.rs** | ~260 | `McpSSHCommands` MCP tool implementations |
