# SSH MCP Flow Diagrams (v4.8.0)

Sequence diagrams for the most common workflows on top of the v4.8.0 ssh-mcp server. All diagrams are Mermaid and assume the rmcp 1.6 transport (HTTP via axum 0.8 or stdio). The v4.x hexagonal layout puts the entry-point `ServerHandler` under `src/infra/mcp/server.rs`, the per-resource debouncer + per-peer cursor under `src/adapters/subscription/memory_registry.rs` (with the transitional global registry at `src/adapters/subscription/legacy.rs`), and the lock-free PTY / command / transfer carriers under `src/adapters/{ssh,sftp}/internal/*` after the H17.6 deep decouple — see [ARCHITECTURE.md](./ARCHITECTURE.md#v41-deep-decouple-completed). v4.5 turned the long-promised `_meta` envelope into the live wire payload of every `resources/read` and derived a stable `PeerId` from `Mcp-Session-Id` (HTTP) / stdio singleton. v4.6 layered the LLM steering surface (NEXT advisory line, AGENT_ID rename, subscribe-first HINTs, cost hints, JSON Schema defaults, wired icon). v4.7 added the MCP inter-tool conversation surface — `structured_content` JSON parallel to Markdown, `resources/templates/list`, `notifications/progress` during long async waits, three new tools (`ssh_run` / `ssh_execute_batch` / `ssh_disconnect_many`), `prompts/list` + `prompts/get` catalog, idempotency cache via `_meta.idempotency_key`, NOT_FOUND closest-match suggestions, optional `INITIAL_BUFFER` line on `ssh_shell_open`. **v4.8 lifts typed `output_schema` advertisement to all 21 tools** — strictly additive on `tools/list` metadata; the subscribe pipeline, request envelopes (`_meta.idempotency_key`, `_meta.progressToken`), and these sequence diagrams stay unchanged. See [API.md](./API.md), [LLM_GUIDE.md](./LLM_GUIDE.md), and [RESOURCES.md](./RESOURCES.md).
