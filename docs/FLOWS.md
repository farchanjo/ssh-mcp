...
Sequence diagrams for the most common workflows on top of the v4.8.0 ssh-mcp server. All diagrams are Mermaid and assume the rmcp 1.6 transport (HTTP via axum 0.8 or stdio). The v4.x hexagonal layout puts the entry-point `ServerHandler` under `src/infra/mcp/server.rs`, the per-resource debouncer + per-peer cursor under `src/adapters/subscription/memory_registry.rs`, and the lock-free PTY / command / transfer carriers under `src/adapters/{ssh,sftp}/internal/*` — see [ARCHITECTURE.md](./ARCHITECTURE.md). For per-version wire surface details: [API.md](./API.md), [LLM_GUIDE.md](./LLM_GUIDE.md), [RESOURCES.md](./RESOURCES.md).

> **v4.8** lifts typed `output_schema` to all 21 tools — strictly additive on `tools/list` metadata. The subscribe pipeline, request envelopes (`_meta.idempotency_key`, `_meta.progressToken`), and these sequence diagrams are byte-identical to v4.7.1.
