# SSH MCP Flow Diagrams (v4.6.0)

Sequence diagrams for the most common workflows on top of the v4.6.0 ssh-mcp server. All diagrams are Mermaid and assume the rmcp 1.6 transport (HTTP via axum 0.8 or stdio). The v4.x hexagonal layout puts the entry-point `ServerHandler` under `src/infra/mcp/server.rs`, the per-resource debouncer + per-peer cursor under `src/adapters/subscription/memory_registry.rs` (with the transitional global registry at `src/adapters/subscription/legacy.rs`), and the lock-free PTY / command / transfer carriers under `src/adapters/{ssh,sftp}/internal/*` after the H17.6 deep decouple — see [ARCHITECTURE.md](./ARCHITECTURE.md#v41-deep-decouple-completed). v4.5 turned the long-promised `_meta` envelope into the live wire payload of every `resources/read` and derived a stable `PeerId` from `Mcp-Session-Id` (HTTP) / stdio singleton. v4.6 leaves the subscribe pipeline and these diagrams unchanged; the LLM steering layer (NEXT advisory line, AGENT_ID rename, subscribe-first HINTs, cost hints, JSON Schema defaults, wired icon) lives in the markdown response shape and the capability handshake — see [API.md](./API.md) and [LLM_GUIDE.md](./LLM_GUIDE.md).
